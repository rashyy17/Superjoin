...
- **Normalize** (pure Python): converts raw values into comparable numbers
  (Cr/Mn/Lakh → INR millions, correctly distinguishing currency from bare
...
- **Link** (embedding similarity): each fact's `subject + attribute + period
  ...
  from the embedded string was a deliberate choice — two facts about the same
  thing with *different* values (a contradiction) should still be found as
  similar, since only their meaning, not their number, should drive matching.
...
1. **Corroborated:** FY24 revenue, ₹8,142 Cr (earnings deck) = ₹81,415 Mn
   (annual report) — same figure, different units, correctly normalized to
   match.
2. **Contradiction explained by context:** Donald Francis Colleran is listed
   ...
   documents.
3. **Apparent contradiction explained by context:** the company's CIN changes
   from U63090DL2011PLC221234 (prospectus) to L63090DL2011PLC221234 (annual
   report). Under India's MCA scheme the first letter encodes listing status
   (U=unlisted, L=listed); the prospectus itself confirms shares were "proposed
   to be listed" at time of writing — the IPO it describes explains the change.
4. **Extraction failure:** FY23 EBITDA was extracted as "₹4,516 million"
   (positive) when the source stated "a loss of ₹4,516 million" — the model
   captured the magnitude but dropped the sign encoded only in prose, not
   notation. Caught via adjudication reasoning when compared against the
...
- **Sign/direction loss:** see Case 4 above. Fix: add an explicit
  `direction` field to the extraction schema, populated whenever the source
  describes a loss, decline, or deficit in prose.
- **Large table truncation:** a handful of dense tables (share-allotment,
  ...
  and were dropped after 3 retries rather than partially corrupted. Next
  step: auto-split oversized blocks by row before extraction.
- **Provider instability:** mid-project, Anthropic usage hit a limit, Gemini's
  free tier had aggressive per-minute/per-day caps, and Groq deprecated the
  model we'd switched to — we ultimately settled on Gemini with billing
  enabled (Tier 1). A more resilient version would abstract the LLM call
  behind a provider-agnostic interface with automatic failover.
- **Linking by embedding similarity misses some matches:** named-person
  ...
  similarity to surface automatically; the Colleran case (#2) was found via a
...
- **Schema is currently fixed, not fully dynamic:** `attribute` is free text,
  which lets new fact types appear without a schema migration, but there's no
  explicit mechanism yet for evolving structured fields as new categories of
  fact emerge.
- **Not yet run on the full dataset:** the India macroeconomy dataset (Economic
  Survey, RBI Annual Report, IMF Article IV) uses the identical pipeline but
  was still processing at submission time due to the rate-limit issues above;
  [add final status here before submitting].

## Additional Notes

Built end-to-end with Claude assisting on every step — architecture decisions,
debugging live API errors (rate limits across three providers), and code
review. All prompts and reasoning are visible in the project's development
history if useful context for evaluation.